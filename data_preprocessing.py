...
def split(t):
    data_merged = pd.read_csv("final_merged_rfe_smote_Progression occurrence.csv")
    data_transcriptomics = data_merged[data_merged.columns[:34]]
    data_radiomics = data_merged[data_merged.columns[34:59]]
    data_pathomics = data_merged[data_merged.columns[59:-1]]
    data_label = data_merged[data_merged.columns[-1]]
    if t == "bs":
        label = "Best response"
    elif t == "ps":
        label = "Potential status"
    elif t == "po":
        label = "Progression occurrence"
    data_transcriptomics[label] = data_label
    data_radiomics[label] = data_label
    data_pathomics[label] = data_label
    data_pathomics.to_csv("Data//pathomics_rfe_smote_Progression occurrence.csv", index=False)
    data_radiomics.to_csv("Data//radiomics_rfe_smote_Progression occurrence.csv", index=False)
    data_transcriptomics.to_csv("Data//transcriptomics_rfe_smote_Progression occurrence.csv", index=False)

def generate_synthetic_data(ratio, target, k):
    ...
    if target == "bs":
        ...
        label = progression_occurrence
    X_train, X_test, y_train, y_test = train_test_split(data, label, train_size=ratio, stratify=label)
...
def get_synthetic_data(ratio=0.6, k=2):
    train_set_bs, test_set_bs = generate_synthetic_data(ratio, "bs", k)
    train_set_po, test_set_po = generate_synthetic_data(ratio, "po", k)
    train_set_ps, test_set_ps = generate_synthetic_data(ratio, "ps", k)
...
if __name__ == "__main__":
    ...
    # x = [2,3,4,5,6]
    # y_cr = [0.281, 0.203, 0.153, 0.117, 0.114]
    # y_kr = [1.55, 1.78, 2.01, 2.20, 2.22]
    # y_sk = [0.43, 0.44, 0.52, 0.58, 0.63]
    # plt.plot(x, y_cr, c="blue", label="Synthetic mean correlation")
    # plt.scatter(5,0.119,c="blue", marker="*", label="original mean correlation", linewidths=3)
    # plt.plot(x, y_kr, c="green", label = "Synthetic mean Kurtosis")
    # plt.scatter(5, 2.21, c="green", marker="*", label="original mean Kurtosis", linewidths=3)
    # plt.plot(x, y_sk, c="red", label = "Synthetic mean Skewness")
    # plt.scatter(5, 0.58, c="red", marker="*", label="original mean Skewness", linewidths=3)
    # plt.legend()
    # plt.xticks(x)
    # plt.xlabel("k")
    # plt.ylabel("indicator value")
    # plt.title("indicator values for Synthetic and original data vs k for SMOTE")
    # plt.show()
    generate_synthetic_data(0.4, "ps")
